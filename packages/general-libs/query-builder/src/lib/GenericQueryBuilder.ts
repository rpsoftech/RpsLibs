...
import { QueryinputObject } from './interfaces';
...
export abstract class GenericQueryBuilder {
  ...
  returning_ids = [];
  // constructor() {}
  _array_values(item: QueryinputObject) {
  ...
  _clear_array(a: any, debug = false) {
    if (debug === true) {
  ...
  private _extract_having_parts(key: string, key_array: any) {
    let m;
  ...
  private _prepare_for_limit_and_offset(item: string | number, type = 'limit') {
  ...
  private _escape_identifiers(
    ...
    if (
      ...
      throw `Please Pass String. You Passed ${item}`
    }
  }
